import './js/helpers/bootstrap';
import './js/helpers/_theme';
import './js/helpers/_font_size';
import './js/helpers/_swiper_iniciable';
import './js/helpers/_stories_view';
import './js/scripts/btn_aside_show';
import './js/scripts/input_search_mobile';
import './js/scripts/btn_accessibility';

import './styles/app.css';
import './styles/header-transparencia.css';
import './styles/news.css';
import './styles/dashboard_home.css';
import './styles/stories_view.css';
import './styles/breadcrumb.css';
import './styles/header_default.css';
import './styles/footer_default.css';
import './styles/icon.css';
import './styles/input_search.css';
import './styles/btn_acessibility.css';
import './styles/transparence.css';
import './styles/licitation_view.css';
import './styles/table_default.css';
import './styles/_new_card.css';

import './bootstrap';
